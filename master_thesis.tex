...
$\R_1,\R_2$を\RC とする。
\Rf とは、強対称モノイド函手 $F:\R_1\to\R_2$ であって、下の図式を可換にするもののこと。
...
\S\arabic{section}.3 でその商として$\Lambda[\M]$を定義し、そのwell-defined性を証明する。
\S\arabic{section}.4では構成した圏$\Lambda[\M]$が\CC になっていることを証明する。
...
\paragraph{Definition \arabic{section}.\arabic{subsection}.5: local category of $\calLambda(\M)$}\ \\
...
以上で中間構造$\calLambda(\R)$の定義が完了した。次節で圏 $\Lambda(\M)$を定義し、次々節でそのデカルト構造を示す。
...
\begin{proof}
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[width=5cm]{2. f.png}\\
        \includegraphics[width=5cm]{3. f_comp_a_tens_delta.png}
        \includegraphics[width=5cm]{4. f_comp_delta_tens_delta.png}\\
        \includegraphics[width=5cm]{5. f_comp_delta_tens_delta_comp_gamma.png}\\
        \includegraphics[width=5cm]{6. f_comp_delta.png}\\
        \includegraphics[width=5cm]{7. delta_comp_f_tens_f.png}\\
        \includegraphics[width=5cm]{8. delta_comp_x_tens_f.png}\\

...
この普遍性を用いた定義は、次のような定義と等価であることが知られている(\cite{Fox76}, \cite{heunen2019categories}, Thm4.28)。
...
この普遍的な構成、構成$L$(named by \cite{Andr_s_Mart_nez_2023})は、与えられた対称モノイド圏の射を余分に増やしたのち適切な同値関係で割る構成である。
...
\begin{proof}
    ...
    次の図式３の可換性を示せば、$\bar{F}([A,f])=\bar{F}([B,g])$であることが分かる。
...
\begin{proof}
    ...
    ３ステップに分けて示す。\\
    (i) 逆写像となるべき写像$\psi:\C(X,Y)\to\cL(\bar{X},\bar{Y})$を構成する。\\

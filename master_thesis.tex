...
\begin{definitionsub}[$\Lambda(\R)$の射]
    ...
    $\calLambda(\R)(\bar{X},\bar{Y})$上の同値関係$\sim$を二項関係「２射で繋がれている」が生成する同値関係とする。
...
\begin{propositionsub}[$\Lambda{[\R]}$の直積]
    ...
    $\bar{X},\bar{Y}\in\C$に対し、$(\bar{X\tensor Y}, \pi_X, \pi_Y)$は$X$と$Y$の直積である。
    ただし、$Y$への射影$\pi_Y:X\tensor Y\to Y$は、$\pi_Y:=[X,id_{X\tensor Y}]$で定める。
    $X$への射影$\pi_X:X\tensor Y\to X$は、$\pi_X:=[Y,\gamma_{X,Y}]$で定める。
\end{propositionsub}
...
\begin{propositionsub}[$\Lambda(\R)$の終対象]
    ...
    \begin{align*}
        &(A,f)\\
        = &\ (A,\l_X\comp (f'\times I))\\
        \sim &\ (X, \l_X) (\text{\refprop{prop:trivial_2-morphism}})
    \end{align*}
    である。
\end{propositionsub}
...
前節で定義した構成$\Lambda$は、関連圏からデカルト圏を得る普遍的な方法になっている。今節ではこの普遍性を示す。

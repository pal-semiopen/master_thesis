...
\begin{proof}
    ...
    \end{align*}
    である。ただし、上式中の$(*1), (*2)$の部分は以下に示す。まず$(*1)$は次のように示される。
    \begin{align*}
        &\ (C\tensor Y\tensor C\tensor X,\  
            \Delta_Z\comp((h\comp(C\tensor\gamma_{X,Y}))\tensor h)
            \comp(CY\tensor\gamma_{X,CX}\tensor Y))\\
        ...
        = &\ (C\tensor C\tensor Y\tensor X,\
            h\comp(\Delta_{C}\tensor\Delta_{XY})
    ...
    \refprop{prop:left_whiskering}と$\tensor$の函手性より、
    $(Y\tensor C\tensor X,\  
        \gamma_{X,Y}\tensor CXY
        \comp(Y\tensor\gamma_{X,CX}\tensor Y))
    \sim (C\tensor Y\tensor X,\  
        (\gamma_{XY,C}\tensor XY)
        \comp(C\tensor\gamma_{X,YX}\tensor Y))$
    であることを示せばよい。
    \begin{align*}
        &\ (Y\tensor C\tensor X,\  
            \gamma_{X,Y}\tensor CXY

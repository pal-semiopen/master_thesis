...
例えば$\Set^{\mono}$は自然な方法で\RC の構造を持ち、
...
集合と単射の圏$\Set^{\mono}$は自然な方法で\RC とみなせるのであった。
この事実は次の２つから従うことだと理解できる。
まず、全ての\CC は自然な方法で\RC とみなせる。
また、\CC のモノ射は直積で閉じていて複製はモノ射なので、モノ射の全体は「\subRC 」 になる。

    
\begin{example}[\CC]
    全ての\CC は自然な方法で\RC とみなせる。特に、$\Set$は\RC である。%また全てのbounded semi-lattice も\RC である。    
\end{example}
...
圏${\Lambda[\R]}$を定義した後に ${\Lambda[\R]}$が持つ自然なモノイド構造がデカルト積を与えることを示す。
...
そこで本稿でも、同値関係で割る前の射集合を持つ中間構造$\B(\R)$を構成し、その商として$\Lambda[\R]$を定義する。
...
\S\ref{subsection:calLambda} では、まずは中間構造$\B(\R)$を定義する。
...
%メモ：well-definednessの証明が必要ない部分が$\B(\R)$
...
\begin{definitionsub}[恒等1-射]
    $X\in\R$とする。
    $\bar{X}$から$\bar{X}$への$\calLambda(\R)$の1-射$id_{\bar{X}}$を $id_{\bar{X}}:=(I, {\lambda_{X}}^{-1})$と定める。
...
一方で、$\Set^{\mono}$の射はそれ自体自然に$\Lambda[\Set^{\mono}]$の射だとみなせるべきである。
...
\begin{lemmasub}[$\R$の射のグラフは${\Lambda[\R]}$ではもとの $\R$の射と同じ]
    ...
    分かりやすさのために図示すれば以下のよう：
    % https://q.uiver.app/#q=
    \[\begin{tikzcd}
        & {(X} && {A\tensor Y)} \\
        {} & {(X} && {X\tensor X} && {X\tensor A \tensor Y).}
        \arrow["f", from=1-2, to=1-4]
        \arrow["{\Delta_X}", from=2-2, to=2-4]
        \arrow["{X\tensor f}", from=2-4, to=2-6]
...
\begin{proof}
    ...
    \begin{figure}[H]
        \begin{align*}
            &\includegraphics[keepaspectratio, width=0.3\linewidth]{11_f1_comp_a1_tens_g.png}\\
            \overset{\text{\reflem{lem:graph_is_same_as_original}}}{\sim}
                &\includegraphics[keepaspectratio, width=0.3\linewidth, valign=c]{12_f1_comp_a1_tens_delta_comp_a1y_tens_g.png}\\
            \overset{\text{\refprop{prop:trivial_2-morphism}}}{\sim}
                &\includegraphics[keepaspectratio, width=0.3\linewidth, valign=c]{13_f1_comp_a1_tens_delta_comp_k_tens_g.png}\\
            \overset{\text{$k$は2-射}}{=}
                &\includegraphics[keepaspectratio, width=0.3\linewidth, valign=c]{14_f2_comp_a2_tens_delta_comp_a2y_tens_g.png}\\
            \overset{\text{\reflem{lem:graph_is_same_as_original}}}{\sim}
                &\includegraphics[keepaspectratio, width=0.3\linewidth, valign=c]{15_f2_comp_a2_tens_g.png}
        \end{align*}
        \caption{string diagramによる証明}
        \label{fig:compose_string}
    \end{figure}
\end{proof}
...
\begin{definitionsub}[${\Lambda[\R]}$の恒等射]
    ...
    各$\bar{X}\in{\Lambda[\R]}$に対し、$id_{\bar{X}}:=[id_{\bar{X}}] (=[I,\lambda_X^{-1}])$と定める。
\end{definitionsub}
...
\begin{definitionsub}[${\Lambda[\R]}$のモノイド構造]
    ...
    単位対象は $I':=I$で定める。
%整合性の同型$\alpha', \lambda', \rho'$及び捻り同型$\gamma'$を自然な方法で定めると、$({\Lambda[\R]},\tensor')$
\end{definitionsub}
...
\begin{proof}
    ...
    \end{align*}
    である。
    $[C,h] = [C\tensor Y\tensor C\tensor X, \ (h\comp(C\tensor\gamma_{X,Y}))\boxdot h]$であることを示す。
    $(C,h) \sim (C\tensor Y\tensor C\tensor X, \ (h\comp(C\tensor\gamma_{X,Y}))\boxdot h)$であることを示せばよい。
    \begin{align*}
        &\ (C\tensor Y\tensor C\tensor X, \ (h\comp(C\tensor\gamma_{X,Y}))\boxdot h)\\
        = &\ (C\tensor Y\tensor C\tensor X,\  
            h\comp\Delta_{CXY}\comp(C\tensor\gamma_{X,YCX}\tensor Y)
            \ \ (\text{後述(*1)より})\\
        \sim &\ (C\tensor C\tensor Y\tensor X,\  
            h\comp\Delta_{CXY}\comp(C\tensor\gamma_{XY,C}\tensor XY)
            \comp(CC\tensor\gamma_{X,YX}\tensor Y))
            \ \ (\text{後述(*2)より})\\
        \sim &\ (C\tensor Y\tensor X,\
            h\comp(C\tensor\Delta_{XY})
            \comp(C\tensor\gamma_{X,XY}\tensor Y))
            \ \ (\text{後述(*3)より})\\
        \sim &\ (C\tensor X\tensor Y,\  h\comp(C\tensor\Delta_{XY}))
            \ \ (\text{後述(*4)より})\\
        \sim &\ (C,\  h)
            \ \ (\text{\refprop{prop:diagonal_2-morphism} と \refprop{prop:left_whiskering}より})
    \end{align*}
    である。
    ただし、上式中の$(*1), (*2), (*3), (*4)$の部分は以下に示す。
    まず$(*1)$は次のように示される。
    \begin{align*}
        &\ (C\tensor Y\tensor C\tensor X, \ (h\comp(C\tensor\gamma_{X,Y}))\boxdot h)\\
        = &\ (C\tensor Y\tensor C\tensor X,\  
            \Delta_Z\comp((h\comp(C\tensor\gamma_{X,Y}))\tensor h)
            \comp(CY\tensor\gamma_{X,CX}\tensor Y))
            \ \ (\text{$\boxdot$の定義より})\\
        = &\ (C\tensor Y\tensor C\tensor X,\  
            \Delta_Z\comp(h\tensor h) \comp(C\tensor\gamma_{X,Y}\tensor CXY)
            \comp(CY\tensor\gamma_{X,CX}\tensor Y))
            \ \ (\text{$\tensor$の函手性より})\\
        = &\ (C\tensor Y\tensor C\tensor X,\  
            h\comp\Delta_{CXY}\comp(C\tensor\gamma_{X,Y}\tensor CXY)
            \comp(CY\tensor\gamma_{X,CX}\tensor Y))
            \ \ (\text{$\Delta$の自然性より})\\
        = &\ (C\tensor Y\tensor C\tensor X,\  
            h\comp\Delta_{CXY}\comp(C\tensor\gamma_{X,YCX}\tensor Y)
    ...
    である。これで$(*1)$が示された。
    続いて$(*2)$を示す。
    \refprop{prop:left_whiskering}と$\tensor$の函手性より、
    $(Y\tensor C\tensor X,\  
        \gamma_{X,YCX}))
    \sim (C\tensor Y\tensor X,\  
        (\gamma_{XY,C}\tensor X)
        \comp(C\tensor\gamma_{X,YX}))$
    であることを示せばよい。
    \begin{align*}
        &\ (Y\tensor C\tensor X,\  
            \gamma_{X,YCX})\\
        = &\ (Y\tensor C\tensor X,\  
            (\gamma_{X,YC}\tensor X)
            \comp(YC\tensor\gamma_{X,X}))
            \ \ (\text{$\gamma$の整合性より})\\
        \sim &\ (C\tensor Y\tensor X,\  
            (\gamma_{X,YC}\tensor X)
            \comp(YC\tensor\gamma_{X,X})
            \comp(\gamma_{Y,C}\tensor XX))
            \ \ (\text{\refprop{prop:trivial_2-morphism} と \refprop{prop:left_whiskering}より})\\
        = &\ (C\tensor Y\tensor X,\  
            (\gamma_{X,YC}\tensor X)
            \comp(\gamma_{Y,C}\tensor XX)
            ...
            \ \ (\text{$\tensor$の函手性より})\\
        = &\ (C\tensor Y\tensor X,\  
            (X\tensor\gamma_{Y,C}\tensor X)
            \comp(\gamma_{X,CY}\tensor X)
            \comp(YC\tensor\gamma_{X,X}))
            \ \ (\text{$\gamma$の自然性より})\\
        = &\ (C\tensor Y\tensor X,\  
            (X\tensor\gamma_{Y,C}\tensor X)
            \comp(\gamma_{X,C}\tensor YX)
            \comp(C\tensor \gamma_{X,Y}\tensor X)
            \comp(YC\tensor\gamma_{X,X}))
            \ \ (\text{$\gamma$の整合性より})\\
        = &\ (C\tensor Y\tensor X,\ 
            (\gamma_{XY,C}\tensor X)
    ...
    である。これで$(*2)$が示された。
    続いて$(*3)$を示す。
    \refprop{prop:left_whiskering}より、
    $(C\tensor C\tensor Y\tensor X,\  
        \Delta_{CXY}\comp(C\tensor\gamma_{XY,C}\tensor XY)
        \comp(CC\tensor\gamma_{X,YX}\tensor Y))
        \sim (C\tensor Y\tensor X,\
            (C\tensor\Delta_{XY})
            \comp(C\tensor\gamma_{X,XY}\tensor Y))$
    ...
    \begin{align*}
        ...
        = &\ (C\tensor C\tensor Y\tensor X,\
            ...
            \comp(CC\tensor\gamma_{X,XY}\tensor Y))
            \ \ (\text{$\because\Delta$はモノイド自然変換})\\
        = &\ (C\tensor C\tensor Y\tensor X,\
            (C\tensor\Delta_{XY})
            \comp(C\tensor\gamma_{X,XY}\tensor Y))
            \comp(\Delta_{C}\tensor YXXY)
            \ \ (\text{$\because\tensor$の函手性})\\
        \sim &\ (C\tensor Y\tensor X,\
            (C\tensor\Delta_{XY})
            \comp(C\tensor\gamma_{X,XY}\tensor Y))
            \ \ (\text{\refprop{prop:diagonal_2-morphism}より})
    ...
    \refprop{prop:left_whiskering}と$\tensor$の函手性より、
    $(Y\tensor X,\
        \Delta_{XY}\comp(\gamma_{X,XY}\tensor Y))
    \sim (X\tensor Y,\  \Delta_{XY})$
    であることを示せばよい。
    \begin{align*}
        &\ (Y\tensor X,\
            (\Delta_{XY})
            \comp(\gamma_{X,XY}\tensor Y))\\
        \sim &\ (X\tensor Y,\
            (\Delta_{XY})
            \comp(\gamma_{X,XY}\tensor Y)
            \comp(\gamma_{Y,X}\tensor XY))
            \ \ (\text{\refprop{prop:trivial_2-morphism}より})\\
        = &\ (X\tensor Y,\
            (\Delta_{XY})
            \comp(X\tensor\gamma_{Y,X}\tensor Y)
            \comp(\gamma_{X,X}\tensor YY)
            \comp(X\tensor\gamma_{X,Y}\tensor Y))
            \ \ (\text{$\gamma$の整合性より})\\
        = &\ (X\tensor Y,\
            (\Delta_{X}\tensor\Delta_{Y})
            \comp(\gamma_{X,X}\tensor YY)
            \comp(X\tensor\gamma_{X,Y}\tensor Y))
            \ \ (\text{$\because\Delta$はモノイド自然変換})\\
        = &\ (X\tensor Y,\
    ...
    \end{align*}
    である。これで$(*4)$が示された。
    これで$(C,h) \sim (C\tensor Y\tensor C\tensor X, \ (h\comp(C\tensor\gamma_{X,Y}))\boxdot h)$の証明が完了した。
    以上で$[C,h]=\psi(\phi([C,h]))$であることの証明が完了した。
    以上で示された。
\end{proof}
...
\begin{proof}
    任意に$\bar{X}\in{\Lambda[\R]}$を取り、$!_{\bar{X}}:=[X,\rho_X]:\bar{X}\to I$と置く。
    任意の$[A,f]:\bar{X}\to I$に対し、次のように$[A,f]=!_{\bar{X}}$であることを示す。
    $(A,f)\sim(X,\rho_X)$であることを示せばよい。

    $f':X\to A$を$f':=(-\tensor I)^{-1}(f\comp\rho_X)$と定めると、
    \begin{align*}
        &(A,f)\\
        = \ & (A,\rho_X\comp (f'\times I))\\
        \sim\ & (X, \rho_X) \ (\text{\refprop{prop:trivial_2-morphism} より})
    \end{align*}
    である。以上で示された。
\end{proof}
...
\begin{proof}
    ...
    \begin{figure}[H]
        ...
        \label{fig:proof_of_well-definedness_of_barF}
    \end{figure}
    ...
    これで図式\ref{fig:proof_of_well-definedness_of_barF}の上面五角形の可換性が示された。
    下面左の五角形の可換性も同様に示される。
    以上を以て図式\ref{fig:proof_of_well-definedness_of_barF} の手前以外の全ての面の可換性が示されたので、手前面の可換性が従う。
    ...
    \begin{figure}
        ...
        \[\begin{tikzcd}
            ...
            \arrow["{F(\lambda_Y)}"', from=1-3, to=2-3]
            \arrow["{\epsilon\times F(Y)}", shift left=3, from=2-5, to=1-5]
            \arrow["{\lambda_{F(Y)}}"', from=2-5, to=1-7]
            \arrow["{\lambda_{F(Y)}}"', from=2-5, to=2-3]
    ...
    \begin{align*}
        ...
        =\ & F(f')\comp\mu_{I,Y}^{-1}\comp(!_{F(I)}\times F(Y))\comp\lambda_{F(Y)}\\
        =\ & F(f')\comp\mu_{I,Y}^{-1}\comp(\epsilon\times F(Y))^{-1}\comp\lambda_{F(Y)}\\
        =\ & F(f')\comp F(\lambda_Y)\comp\lambda_{F(Y)}^{-1}\comp\lambda_{F(Y)}\\
        =\ & F(f)\comp\lambda_{F(Y)}^{-1}\comp\lambda_{F(Y)}\\
        =\ & F(f)
    \end{align*}
    である。これで示された。
    ...
    $[A,f]:\bar{X}\to \bar{Y} \rmin \LambdaR$を１つ取り固定する。$\bar{\Eta_{\R}\comp G}([A,f]) = G([A,f])$であることを示す。
    \begin{align*}
        &\bar{\Eta_{\R}\comp G}([A,f])\\ 
        =\ & (\Eta_{\R}\comp G)(f)\comp\mu_{A,Y}^{-1}\comp\pi_2\\
        =\ & (\Eta_{\R}\comp G)(f)\comp(\nu_{\Eta_{\R}(X),\Eta_{\R}(Y)}\comp G(m_{A,Y}))^{-1}\comp\pi_2\\
        =\ & (\Eta_{\R}\comp G)(f)\comp G(m_{A,Y})^{-1}\comp\nu_{\Eta_{\R}(X),\Eta_{\R}(Y)}^{-1}\comp\pi_2\\
        =\ & G([I,f\comp\lambda_Y])\comp G(m_{A,Y})^{-1}\comp\nu_{\Eta_{\R}(X),\Eta_{\R}(Y)}^{-1}\comp\pi_2\\
        =\ & G([I,f\comp\lambda_Y])\comp G(m_{A,Y})^{-1}\comp G(\pi_2)\\
        =\ & G([I,f\comp\lambda_Y]\comp (m_{A,Y})^{-1}\comp \pi_2)\\
        =\ & G([A,f\comp\lambda_Y\comp {\lambda_Y}^{-1}])\\
        =\ & G([A,f])\\
    \end{align*}
    以上で示された。分かりやすさのために、同じ式変形の図式的表示を付す。
...
本研究室の山本雄太氏、洞龍弥氏には、
    ...
    深く感謝する。
同大学院工学系研究科精密工学専攻医用精密工学研究室箕浦有希也氏には、
    論文の推敲に多大な助言をいただいた。
    深く感謝する。

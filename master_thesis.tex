...
\newtheorem{definitionsub}{定義}[subsection]
\newtheorem{propositionsub}[definitionsub]{命題}
\newtheorem{theoremsub}[definitionsub]{定理}
\newtheorem{lemmasub}[definitionsub]{補題}
\newtheorem{examplesub}[definitionsub]{例}
...
\topic{本論文で主に扱うのは、与えられた\RC からそれをよい\subRC に持ちうる\CC を構成する普遍的な操作$\Lambda$である。}
操作$\Lambda$は、あらゆる\RC $\R$に対して\CC $\Lambda(\R)$ 及び函手$\Eta_{\R}:\R \to \Lambda(\R)$の組$(\Lambda(\R), \Eta_{\R})$を１つずつ返すものである。この$\Eta_{\R}$は必ずしも単射的(即ち忠実かつ対象上本質的に単射)であるとは限らないが、もしも$\R$が何か\CC $\C$のよい\subRC として構成されているときには、必ず$\Lambda(\R)\simeq \C$である。特に、$\R$をよい\subRC に持つ\CC $\C$は同型を除いて一意的であることが従う。
...
\S\thesection.2 で構成の第一段階として双圏様の中間構造を定義し、
\S\thesection.3 でその商として$\Lambda[\R]$を定義し、そのwell-defined性を証明する。
\S\thesection.4では構成した圏$\Lambda[\R]$が\CC になっていることを証明する。
...
そのため、最終的に得る圏のwell-defined性の証明(prop \arabic{section}.3.4)が書かれている。
...
lemma \arabic{section}.\arabic{subsection}.3に先んじて、その意味を説明しておく。
...
\begin{proof}
    ...
    \begin{align*}
        & (A,f)\\
        \sim \ & (A\tensor Y,\ f\comp(A\tensor \Delta_Y))  \ \ \text{(prop \arabic{section}.2.4)より}\\
        \sim \ & (A\tensor A\tensor Y,\ f\comp(A\tensor \Delta_Y)\comp(\Delta_A\tensor YY))
            \ \ \text{(prop \arabic{section}.2.3 and \arabic{section}.2.7より)}\\
...
\begin{proof}
    ...
    \begin{align*}
        & (A_1,f_1)\comp(B,g)\\
        = \ & (A_1\tensor B, f_1\comp(A_1\tensor g)) \ \ \text{(by definition of composition)}\\
        \sim \ & (A_1\tensor Y\tensor B, f_1\comp(A_1\tensor \Delta_Y)\comp(A_1 Y\tensor g))
            \ \ \text{(by prop \arabic{section}.3.3 and lem \arabic{section}.2.7)}\\
        \sim \ & (A_2\tensor Y\tensor B, f_1\comp(A_1\tensor \Delta_Y)\comp(A_1 Y\tensor g))\comp(k\tensor BZ)
            \ \ \text{(by prop \arabic{section}.2.3 and lem \arabic{section}.2.7)}\\
        ...
        = \ & (A_2\tensor Y\tensor B, f_2\comp(A_2\tensor \Delta_Y))\comp(A_2 Y\tensor g)
            \ \ \text{(because $k$ is a 2-morphism from $(A_1,f_1)$ to $(A_2,f_2)$)}\\

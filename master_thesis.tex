...
$\R_1,\R_2$を\RC とする。
\Rf とは、強対称モノイド函手 $F=(F, \theta, \epsilon):\R_1\to\R_2$ であって、下の図式を可換にするもののこと。
...
このとき、任意の\CC \ $\D$ 及び\Rf $F=(F,\theta, \epsilon):\R \to \D$に対し、\\
...
\begin{proof}
    ...
    (i) ($\bar{F}$の構成とwell-definednessの証明)
    \Rf $F=(F,\theta, \epsilon):\R \to \D$に対し
    （強）モノイド函手 $\bar{F} = (\bar{F}, \bar{\theta}, \bar{\epsilon}):\C\to\D$
    を対応させる操作$\bar{(-)}$を次で定める：\\
    各$\bar{X}\in\C$に対し、$\bar{F}(\bar{X}):=F(X)$.\\
    各$[A,f]:\bar{X}\to \bar{Y}\ \text{in}\ \C$に対し、函手$\bar{F}([A,f])$を
    $\bar{F}([A,f]):=F(f)\comp\theta_{A,Y}^{-1}\comp\pi_{F(A),F(Y)}$で定める。\\
    ただし、$\theta_{A,Y}:F(A)\times F(Y) \to F(A\tensor Y)$はFのモノイド函手としての構造の同型である。\\
    ...
    また$\bar{\theta}_{\bar{F}(\bar{X}),\bar{F}(\bar{Y})}:\bar{F}(\bar{X})\times\bar{F}(\bar{Y})\to \bar{F}(\bar{X}\tensor)$を
    $\bar{\theta}_{\bar{F}(\bar{X}),\bar{F}(\bar{Y})}:=\theta_{F(X),F(Y)}$、$\bar{\epsilon}:=\epsilon$と定める。
    すると$(\bar{F},\bar{\theta},\bar{\epsilon})$は確かに強モノイド函手である。\\
    %ほぼ自明なので特に説明しない
    ...
    \begin{align*}
        ...
        =& \bar{F}(\Eta_{\R}(f))\\
        =& \bar{F}([1,f'])\\
        =& F(f')\comp\theta_{I,Y}^{-1}\comp\pi_{F(I),F(Y)}\\
        =& F(f')\comp\theta_{I,Y}^{-1}\comp(\!\times F(Y))\comp\rho_{F(Y)}\\
        =& F(f')\comp\theta_{I,Y}^{-1}\comp(\epsilon\times F(Y))^{-1}\comp\rho_{F(Y)}\\
    ...
    \[\begin{tikzcd}
        ...
        \arrow["{\theta_{1,Y}}"', from=1-5, to=1-3]
    ...
    (ii-b) （強）モノイド函手 $G:\C\to\D$に対し、$\bar{\Eta_{\R}\comp G}=G$であることを示す。\\

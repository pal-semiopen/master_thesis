...
このとき、任意の\CC \ $\D$ 及び\Rf $F=(F,\theta):\R \to \D$に対し、\\
...
\begin{proof}
    ...
    (i) ($\bar{F}$の構成とwell-definednessの証明)
    \Rf $F:\R \to \D$に対し（強）モノイド函手 $\bar{F}:\C\to\D$を対応させる操作$\bar{(-)}$を次で定める：\\
    各$\bar{X}\in\C$に対し、$\bar{F}(\bar{X}):=F(X)$.\\
    各$[A,f]:\bar{X}\to \bar{Y}\ \text{in}\ \C$に対し、$\bar{F}([A,f])$を
    $\bar{F}([A,f]):=F(f)\comp\theta_{A,Y}\comp\pi_{F(A),F(Y)}$で定める。\\
    ...
    これで$\bar{F}$のwell-definednessが示された。\\
    また、$\bar{F}$は確かに強モノイド函手である。\\
    %ほぼ自明なので特に説明しない
    ...
    \begin{align*}
        &(\Eta_{\R}\comp\bar{F})(f) 
        =& \bar{F}(\Eta_{\R}(f))\\
        =& \Eta_{\R}(F(f)\comp\theta_{I,Y}\comp\pi_{F(I),F(Y)})\\
        =& [I,F(f)\comp\theta_{I,Y}\comp\pi_{F(I),F(Y)}\comp\rho_Y]\\
        =& [I,F(f)\comp\theta_{I,Y}\comp F(\rho_Y)\comp\rho_Y]\\
        =& [I,F(f)\comp\theta_{I,Y}\comp F(\rho_Y\comp\rho_Y)]\\
        =& [I,F(f)\comp\theta_{I,Y}\comp F(\rho_Y)]\\
        =& [I,F(f)\comp\theta_{I,Y}]\\
        =& [I,F(f)]\\
        =& F(f)
    \end{align*}

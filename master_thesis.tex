...
\begin{definitionsub}[恒等1-射]
    $X\in\R$とする。
    $\bar{X}$から$\bar{X}$への$\calLambda(\R)$の1-射$id_{\bar{X}}$を $(I, \rho_{X})$と定める。
...
\begin{proof}
    ...
    \begin{align*}
        ...
        =\ & G([I,f\comp\rho_Y])\comp G(m_{A,Y})^{-1}\comp\nu_{\Eta_{\R}(X),\Eta_{\R}(Y)}^{-1}\comp\pi_2\\
        =\ & G([I,f\comp\rho_Y])\comp G(m_{A,Y})^{-1}\comp G(\pi_2)\\
        =\ & G([I,f\comp\rho_Y]\comp (m_{A,Y})^{-1}\comp \pi_2)\\
        =\ & G([A,f\comp\rho_Y\comp {\rho_Y}^{-1}])\\
        =\ & G([A,f])\\
    \end{align*}

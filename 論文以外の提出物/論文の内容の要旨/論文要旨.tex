...
本研究は、対称モノイド圏に複製あるいは対角射の構造を付加した構造である関連圏の構造を分析することを目標としている。
関連圏は、対称モノイド圏とデカルト圏の中間構造としてJacobsによりアフィン圏とともに導入された。


本論文では、関連圏からデカルト圏を構成する左普遍的な方法の有限的な表示である構成$\Lambda$を与え、
    それがデカルト圏$\C$に対し$\Lambda[\C^{\mono}]\simeq \C$となる性質を示すことを証明する。
...
第２節ではまず初めに、関連圏とその周辺概念を定義し、重要な例を挙げる。
    関連圏は、対称モノイド圏$\R$とその上のモノイド自然変換$\Delta:-\to -\tensor - $の組$(\R,\Delta)$であって、
    $\Delta$が可換性・結合性の２つを満たすものである。

第３節で構成$\Lambda$の定義を行い、そのwell-defined性を示す。
    構成$\Lambda$は、関連圏に対しデカルト圏を対応させる操作である。
    関連圏$\R$に対し、$\Lambda[\R]$の対象は$\R$の対象そのものであり、
    $\Lambda[\R]$の射は$\R$の対象と$\R$の射の組の同値類である。
    $\Lambda[\R]$の射の合成のwell-defined性は非自明であるから、これを示す。
    以て圏$\Lambda[\R]$が定義されたのち、$\Lambda[\R]$がデカルト圏であることを示す。
    

第４節では、構成$\Lambda$が関連圏からデカルト圏を得る左普遍的な方法であることを示す。
    すなわち、
...
\begin{example}[\CC]
    全ての\CC は素朴な方法で\RC とみなせる。特に、$\Set$は\RC である。
\end{example}
